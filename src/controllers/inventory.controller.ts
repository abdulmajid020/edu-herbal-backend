    ...
    try {
      ...
      let list = dbInventory.length > 0
        ? dbInventory.map((inv) => ({
            ...
            item: inv.item || inv.product?.name || `Product #${inv.productId}`,
            category: inv.category || inv.product?.category || "General",
            stock: inv.stock,
            min: inv.minLevel,
      ...
      try {
        ...
        if (product && product.inventory) {
          ...
          updatedRecord = {
            ...
            item: updatedInv.item || product.name,
            category: updatedInv.category || product.category,
            stock: updatedInv.stock,
            min: updatedInv.minLevel,
